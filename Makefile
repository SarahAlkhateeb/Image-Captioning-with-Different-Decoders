...
attention_glove:
	bash /opt/local/bin/run_py_job.sh -e dit245_group15 -p cpu -c 32 -s train.py -- 'glove_att' --model 'attention' --batch_size 32 --epochs 2 --use_glove True --fine_tune_embedding True --embed_size 300 --checkpoint 'glove_att_0.pth.tar' --max_caption_length -1 --workers 32
...
attention_bert:
	bash /opt/local/bin/run_py_job.sh -e dit245_group15 -p cpu -c 32 -s train.py -- 'bert_attention' --model 'attention' --batch_size 32 epochs 1 --use_bert True --fine_tune_embedding True --embed_size 768 --max_caption_length -1 --workers 32
clean:
	rm slurm-*
